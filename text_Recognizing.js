...
/**
 *  Function to code what script the code is using 
 *  for a given character code if we can return a SCRIPT then after 
 *  the script is extracted then the comparision will recognize the text
 * 
 */

 function characterScript(code){ //returns the character script
    for(let script of SCRIPTS){
        if ( script.ranges.some(([from,to]) =>{ //input from  first and the last element
 ...
 function countBy(items,groupName){ //counts inside the collections and returns the number of groupName 
    let counts=[]; //counter function
    for(let item of items){
     let name= groupName(item); //counts if item is greater then 2
     let known= counts.findIndex(c => c.name == name); //checks if name is inside counts
      if(known == -1){ //if name is not inside the counts
...
// console.log(countBy([1,2,3,4,5,6,6], n=> n>2));

//using the above function we can write which scripts uses which texts
function textScripts(text){
    let scripts= countBy(text, char => {
        let script = characterScript(char.codePointAt(0)); //finda the script for the matching characters
        return script ? script.name : "none"; //returns the script that matched the critearea
    }).filter(({name}) => name !="none"); //script returned should not contains name==none
 
    let total = scripts.reduce((n, {count}) => n + count,0); //counts the total number of the script matched and assign it to the count variable
    if(total == 0) return "No Script founds";

    return scripts.map(({name, count}) =>{
        return `${Math.round(count * 100 / total)} % ${name}`; //change the arry to new form  and returns the %  matched of each language 
    }).join(", ");
    }

    //checks the character matching 
console.log(textScripts('英国的狗说jsdf,俄罗斯的狗说 "тяв'));